        ...
        while menulist:
            ...
            for (code,desc,type,key) in menulist:
                if type and (code or desc):
                    mtxt += '<%s> %s' % (code,desc)
                    if type in ['info','list','opt','stat']: mtxt += ': %s' % callobj.getAttribute(type,key,default='#!#ERROR#!#')
                    elif type == 'int': mtxt += ': %s' % callobj.getAttribute('stat',key,default='#!#ERROR#!#')
            ...
            while mtxt:
                try:## ~ Input user choice ~~~ ##
                    ...
                    if choicedict.has_key(choice):
                        ...
                        if type == 'info': callobj.info[key] = callobj._editChoice(key,callobj.info[key])
                        if type == 'stat': callobj.stat[key] = callobj._editChoice(key,callobj.stat[key],numeric=True)
                        if type == 'int': callobj.stat[key] = int(callobj._editChoice(key,callobj.stat[key],numeric=True))
                        if type == 'opt': callobj.opt[key] = not callobj.opt[key]
                        if type == 'list': callobj.list[key] = string.split(callobj._editChoice(key,callobj.list[key]))
                        if type == 'infile': callobj.info[key] = rje.getFileName('%s File Name?' % key,callobj.info[key])
                        if type == 'outfile': callobj.info[key] = rje.getFileName('%s File Name?' % key,callobj.info[key],mustexist=False,confirm=True)
                        ...
                        if type == 'addcmd':
                            ...
                            break
                        if type in ['info','list','opt','stat','infile','outfile']:
