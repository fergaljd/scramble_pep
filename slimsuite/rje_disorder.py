...
def makeInfo():     ### Makes Info object
    ...
    (program, version, last_edit, copyright) = ('RJE_DISORDER', '0.7', 'September 2010', '2008')
    description = 'Disorder Prediction Module'
...
#########################################################################################################################
    def iuPred(self,retry=2):     ### Runs IUPred disorder prediction
        ...
        try:
            ...
            name = self.info['Name'][:4] + rje.randomString(4)
            tmp = name + '.tmp'
...
#########################################################################################################################
    def minRegion(self):    ### Reduced self.list['RegionDisorder']/self.list['RegionFold'] using stat['MinRegion']
        ...
        except: self.errorLog('Problem applying MinRegion(%d)' % self.stat['MinRegion'])            
#########################################################################################################################
