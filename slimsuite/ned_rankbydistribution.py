...
import rje_seq, rje_uniprot, rje, rje_blast, rje_slim
...
#########################################################################################################################
    def calculateRankings(self,SLiMFinderObj):    ### Main Class Ranking calculation method
        '''Main Class Ranking calculation method.'''
        try:### ~ [1] Setup ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
            ...
            if self.options["fixes"] == "3":
                ...
                for i in range(3,self.options["maxLength"] + 1):
                    SLiMFinderObj.log.printLog("#NORM","Scoring motifs of length " +str(i),newline=True)
                    count = 0
                    for score in topRanking[i]:
                        for motif in topRanking[i][score]:
                            count += 1
                            self.slimFinderObj.log.printLog("\r#NORM","Motif " + str(count) + " of " + str(len(topRanking[i])),newline=False,log=False)
                            self.sigSlimDist(motif[0],pVal_sig[i][score])
                    SLiMFinderObj.log.printLog("\r#NORM","Motif " + str(count) + " of " + str(len(topRanking[i])),newline=True)
...
#########################################################################################################################
    def collapseList(self,listTemp):
        ...
        return stringTemp
#########################################################################################################################
    def calculateRankingsMeanFix(self,SLiMFinderObj):
        try:
