...
#########################################################################################################################
    def buildPam(self):     ### Builds PAM Matrix in memory
        ...
        try:
            ...
            if rje.checkForFile(self.info['Name']):
                PAMFILE = open(self.info['Name'], 'r')
                file_lines=PAMFILE.readlines()
                PAMFILE.close()
            else:
                text = 'File %s not found.' % self.info['Name']
                self.info['Name'] = rje.makePath(self.info['Path']) + self.info['Name']
                text = '%s Trying %s...' % (text,self.info['Name'])
                if rje.checkForFile(self.info['Name']):
                    PAMFILE = open(self.info['Name'], 'r')
                    file_lines=PAMFILE.readlines()
                    PAMFILE.close()
                else:
                    self.log.errorLog('%s File not found!' % text,printerror=False,quitchoice=True)
                    raise
